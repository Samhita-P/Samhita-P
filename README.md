# Hi there 👋, I'm Samhita Prashanth

### Backend Engineer | Full-Stack Developer | Software Tester

🎓 Computer Science & Engineering @ K S Institute of Technology (CGPA: 8.5)
📍 Bengaluru, India
💼 Associate Software Engineering Intern (SDET) @ Mphasis
🚀 Passionate about Backend Development, Distributed Systems, AI-Powered Applications, and Software Quality Engineering

## 👩‍💻 About Me

I am a Computer Science Engineering student graduating in 2026 with hands-on industry experience in Software Development, Backend Engineering, Full-Stack Development, and Software Testing.

My experience spans:

* Backend Development using Django, DRF, Spring Boot, and REST APIs
* Full-Stack Development with Flutter and modern web technologies
* Authentication & Security using Keycloak, JWT, OAuth2, and OTP
* Database Design with PostgreSQL and MongoDB
* Software Testing using Selenium, TestNG, Cucumber, and Agile practices
* AI/ML Applications using TensorFlow, OpenCV, and Scikit-learn

## 🛠️ Tech Stack

### Languages

Python • Java • SQL • TypeScript • C

### Backend

Django • Django REST Framework • Spring Boot • REST APIs • Microservices

### Frontend

Flutter • Angular • React • HTML • CSS

### Databases

PostgreSQL • MongoDB • SQLite

### DevOps & Tools

Docker • Jenkins • Linux • Git • GitHub • Postman • Jira • Figma

### Testing

Selenium • TestNG • Cucumber • Manual Testing • STLC

## 🚀 Featured Projects

### 🎓 KSIT Nexus

A unified digital campus platform serving 600+ students and faculty.

* 30+ REST APIs
* AI-powered chatbot
* Anonymous Complaint System
* Study Group Finder
* Keycloak SSO + JWT + OTP Authentication

### 🧠 Cognitive AI Platform

Real-time cognitive workload analysis using telemetry-driven machine learning.

* Behavioral analytics
* Workload classification
* Interactive dashboards
* AI-powered recommendations

### ♿ Visual Accessibility & AI Captioning Tools

Accessibility solutions using OpenCV, TensorFlow, and Flask.

* Color blindness correction
* AI image caption generation
* REST API deployment

### 💰 CashBee

Real-time cash delivery platform.

* Django + Flutter
* Stripe payments
* Firebase tracking
* Twilio notifications

## 📚 Research Publications

📄 KSIT Nexus: A Unified Digital Campus Platform (IJARCCE, 2025)

📄 Native Aura: Empowering Karnataka's Tribal Communities via E-Commerce (IJISRT, 2025)

## 🏆 Achievements

* Published 2 peer-reviewed research papers
* Excellent Performance Award at Sisail Pvt Ltd
* Team Lead for Major and Minor Engineering Projects
* Experience across Development, Testing, and AI domains


## 📊 GitHub Stats

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=Samhita-P&show_icons=true&theme=github_dark)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=Samhita-P&layout=compact&theme=github_dark)

## 🏆 GitHub Trophies

[![trophy](https://github-profile-trophy.vercel.app/?username=Samhita-P&theme=onedark)](https://github.com/ryo-ma/github-profile-trophy)

## 📫 Connect With Me

📧 samhita.prashanth@gmail.com

💼 LinkedIn:
https://linkedin.com/in/samhita-prashanth-153437287

🌐 Portfolio:
https://tech-portfolio-chi.vercel.app
